...
src/scaffold/
  ...
  react-vite/
    ...
    templates/
      ...
      copilot-instructions.ts      — getCopilotInstructionFiles(cart): FileMap (general + path-specific Copilot rules)
...
- Every scaffolded project ships with GitHub Copilot custom instructions (`.github/copilot-instructions.md` + path-specific `.github/instructions/*.instructions.md` files following the `applyTo` frontmatter format). The set of emitted files is driven by `getCopilotInstructionFiles(cart)` and changes based on chosen layout (FSD/BPR) and optional libraries (router, state, query, linter)
...
6. **Add a matching Copilot instruction template** in `src/scaffold/react-vite/templates/copilot-instructions.ts` — this step is required, not optional:
   - Write a new template function (e.g. `uiLibraryTemplate(cart)`) returning a markdown string that begins with `frontmatter(applyTo)` scoped to the right file paths for **both** FSD and BPR layouts (consult/extend `FSD_PATHS` and `BPR_PATHS`)
   - The instruction content should describe naming, placement, and usage conventions for the new tool so Copilot can apply them automatically
