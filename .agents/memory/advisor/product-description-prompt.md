...
- The rendered AGENTS.md (`src/scaffold/react-vite/templates/harness-setup.ts` `projectSectionsTemplate`, post-0016) already has a `## Project Overview` section that lists stack only — that is the natural, zero-new-file home for a one-line description. No new config file needed.
- Free-text `input()` prompt pattern already exists (`src/options/harness-only/index.ts` menuProjectName) — adding a description prompt is mechanically trivial; the cost is UX friction + maintenance of one more cart field threaded through types + every harness-setup template, not implementation difficulty.
